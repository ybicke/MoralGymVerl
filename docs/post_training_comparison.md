...
*(2026-08-25. Numbers copied from `eval_results/post_training/<experiment>/analysis/results_*.md`; trace excerpts from the matching `trace_snippets_*.md`. Previous, longer version incl. gemma: git c3ebfd9.)*
...
Verbatim excerpts, checkpoint eval, no moral text in the prompt; payoff enumeration elided. Full excerpts and selection rule in `eval_results/post_training/*/analysis/trace_snippets_*.md`. State D<sub>A</sub>C<sub>O</sub> (agent defected on a cooperator) unless noted — the state both arms move.
