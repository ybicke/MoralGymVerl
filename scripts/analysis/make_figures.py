...
def trace_language_table(args, out_dir: Path) -> None:
    ...
    transfer = dict(parse_labeled(args.transfer))
    rows = []
    ladders = parse_labeled(args.ladder)
    ladders += [(lab, None) for lab in transfer if
                lab not in dict(ladders)]
    for label, ladder in ladders:
        ...
        # PGG transfer columns: final checkpoint of the SAME run in the
        # transfer group (run identity = the experiment's canonical name).
        pgg = ["--", "--"]
        if label in transfer:
            run_name = ladder.parents[1].name if ladder is not None else None
            best = (None, -1)
            for run_dir in discover_run_dirs([transfer[label]]):
                cell = load_cell(run_dir)
                if cell is None:
                    continue
                run, step = checkpoint_of(cell.meta)
                if run == "base":
                    continue
                if run_name is not None and run != run_name:
                    continue
                if step > best[1]:
                    best = (cell, step)
