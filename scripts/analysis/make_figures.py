...
def trace_language_table(args, out_dir: Path) -> None:
    """generated/tables/trace_language.tex: normative-language and
    verbatim-recitation rates per run and checkpoint, pooled over the
    four states (the measures are nearly state-uniform; behaviour
    tables keep the state axis). Normative = trace contains >=1 of the
    12 reviewed stems; recitation = reproduces >=6 consecutive words
    of the principle. Computed from the same ladder cells as the
    checkpoint figures."""
    ...
    cols = [0, 60, 120, None]  # None = each run's final step
    rows = []
    for label, ladder in parse_labeled(args.ladder):
        ...
        refs = reference_rows(model_refs, args.principle) if model_refs else {}
        if "base" in refs:
        ...
        for want_norm in (True, False):
            for c in cols:
                ...
                cell = base_cell if c == 0 else cells.get(step)
                if cell is None:
                    row.append("--")
                else:
                    n, r = rates(cell)
                    row.append(f"{n:.0f}" if want_norm else f"{r:.0f}")
        rows.append(row)

    BS, NL = chr(92), chr(10)
    hdr = ["base", "60", "120", "final"]
    lines = [
        "% generated by scripts/analysis/make_figures.py -- do not edit",
        BS + "begin{table}[H]",
        BS + "centering",
        BS + "caption{Moral language in the reasoning traces, pooled over"
        " the four states (400 traces per cell, checkpoint-eval surface,"
        " no principle in any prompt). " + BS + "emph{Normative} = share"
        " of traces containing at least one of 12 reviewed stems (good"
        " faith, trust, exploit, moral, ethic, principle, fair, reciproc,"
        " wrong, obligat, betray, honest); the untrained base rate is"
        " payoff-talk hits, mostly `exploit'. " + BS + "emph{Recites} ="
        ...
        BS + "begin{tabular}{l" + "r" * 8 + "}",
        BS + "toprule",
        " & " + BS + "multicolumn{4}{c}{normative (" + BS + "%)}"
        " & " + BS + "multicolumn{4}{c}{recites (" + BS + "%)} " + BS + BS,
        BS + "cmidrule(lr){2-5}" + BS + "cmidrule(lr){6-9}",
        "Run & " + " & ".join(hdr + hdr) + " " + BS + BS,
        BS + "midrule",
    ]
    for row in rows:
        lines.append(" & ".join(row) + " " + BS + BS)
    lines += [BS + "bottomrule", BS + "end{tabular}", BS + "end{table}"]
    out = out_dir / "generated" / "tables" / "trace_language.tex"
    out.parent.mkdir(parents=True, exist_ok=True)
...
def prompt_panels(args, out_dir: Path) -> None:
    ...
    (tdir / "game_prompt.txt").write_text(prompt + NL)
    (tdir / "principle.txt").write_text(principle + NL)

    lines = [
        "% generated by scripts/analysis/make_figures.py -- do not edit",
        BS + "lstinputlisting[title={Game prompt (all runs, both"
        " channels; the fabricated previous round varies by episode)}]"
        "{generated/panels/game_prompt.txt}",
    ...
    (out_dir / "generated" / "prompt_panels.tex").write_text(
        NL.join(lines) + NL)
    print(f"wrote {out_dir / 'generated' / 'prompt_panels.tex'}")
