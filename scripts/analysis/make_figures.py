...
def trace_language_table(args, out_dir: Path) -> None:
    ...
    def rates_logged(cell):
        sources.append("%   " + str(cell.run_dir))
        return rates(cell)
        ...
        for want_norm in (True, False):
            for c in cols:
                ...
                row.append("--" if cell is None else
                           f"{rates_logged(cell)[want_norm - 1]:.0f}")
        rows.append(row)
    ...
    src_pd = NL.join(["% source cells:"] + sorted(set(sources))) + NL
    sources.clear()
    (tdir / "trace_language_pd.tex").write_text(note + src_pd + table(
    ...
    src_pgg = NL.join(["% source cells:"] + sorted(set(sources))) + NL
    (tdir / "trace_language_pgg.tex").write_text(note + src_pgg + table(
