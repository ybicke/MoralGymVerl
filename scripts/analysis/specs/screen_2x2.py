...
def readme_header(cells: Dict[CellKey, Path], paths: List[Path]) -> str:
    ...
    return "\n".join([
        ...
        "`scripts/analysis/make_results.py` (LaTeX sources in "
        "`tex/`).",
        "", "",
    ])
...
def add_arguments(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--exemplars", type=int, default=1,
                        help="example traces per arm x state, written beside "
                             "the doc as traces_<model>.md (0 = none); "
                             "fixed-presentation cells only")


def build(args: argparse.Namespace) -> Path:
    """2x2 single-turn screen: state cooperation, probe-B, ablations."""
