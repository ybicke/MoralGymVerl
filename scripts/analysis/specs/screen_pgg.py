...
single-turn screen -- the PGG counterpart to specs/screen_2x2.py, which
...
def load_cells(run_dirs: List[Path]) -> Dict[str, Dict]:
    ...
    time -- `make_results.py <group>/cells/*__list__*`.
    """
    cells: Dict[str, Dict] = {}
...
def header(cells: Dict[str, Dict], arms: List[str], paths: List[Path]) -> str:
    ...
    return "\n".join([
        ...
        "`scripts/analysis/make_results.py` (LaTeX sources in `tex/`).",
        "", "",
    ])
...
def build(args: argparse.Namespace) -> Path:
    """PGG single-turn screen: contribution curve, label valence, arithmetic."""
