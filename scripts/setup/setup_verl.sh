...
echo ""
echo "  2. Generate a training dataset:"
echo "     srun --account=aa004 --partition=debug --time=00:10:00 \\"
echo "          --environment=${EDF_NAME} --pty bash -c \\"
echo "          'pip install -e ~/MoralGymVerl --no-deps && \\"
echo "           python -m moralgym_verl.training.dataset \\"
echo "             --config ~/MoralGymVerl/configs/datasets/grpo_pd_tft.yaml \\"
echo "             --output-dir ~/MoralGymVerl/datasets/grpo_pd_tft \\"
echo "             --n-train 8000 --n-val 256'"
echo ""
echo "  3. Submit a training job:"
echo "     bash ~/MoralGymVerl/scripts/slurm/train_verl.sh grpo_pd_tft my_run_name"
